...
function homeAdd(data) {
    ...
    html += '</div>';
    html += '</li>';
    html += '<li class="col-lg-8 col-md-7 col-sm-6 col-xs-4" >';
    html += '<div>';
...
$(".nav").on("click", function(){
    ...
    $(this).css({"background-color":"rgb(44, 102, 44)", "color":"#fff"});
    $(".section").hide();
    $(".section").eq(n).show();
});
...
function homeDel(obj){
    if(confirm("정말로 삭제할건가요?:)")) {
        // var id = obj.parentNode.parentNode.parentNode.id;
        var id = $(obj).parent().parent().parent().attr("id");
        if(id != ""){db.ref("root/home/"+id).remove();
         } 
    }
}
...
// 최초 페이지가 생성 될떄 한번 실행되며 shop레퍼런스에 콜백을 링크한다. 
function initShop() {
    $(".grid > ul").remove();
...
// chk변수의 값(C, U)에 따라 ul을 생성 또는 수정한다.
...
function shopUp(obj) {
    if(confirm("수정하실건가요 :)\n꼼꼼히 확인해보세요.")){
        var dk = $(obj).parent().parent().parent();
        var id = dk.attr("id");
        var title = $(".title", dk).val();
        var icon = $(".icon", dk).val();
        var color = $(".color", dk).val();
        var link = $(".link", dk).val();
        if(title == ""){
            alert("카테고리 명을 입력하세요:)");
            $(".title", dk).focus();
            return false;
...
function shopDel2(obj) {
    ...
    var sd = $(obj).parent().parent().attr("id");  // li
    db.ref("root/shop/"+id+"/sub/"+sd).remove();
  }
}
...
function shopUp2(obj) {
    if(confirm("수정할까요:)")){
        var id = $(obj).parent().parent().parent().attr("id");
        var sd = $(obj).parent().parent();
        var sdo = sd.attr("id")
        var title = $(".title", sd).val();
        var icon = $(".icon", sd).val();
        var color = $(".color", sd).val();
        var link = $(".link", sd).val();
        if(title == ""){
            alert("카테고리 명을 입력하세요:)");
            $(".title", sd).focus();
            return false;
            ...
            db.ref("root/shop/"+id+"/sub/"+sdo).update({
            title: title,
            icon: icon,
            color: color,
            link: link
